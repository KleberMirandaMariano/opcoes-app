...
const { validateStrategy } = require('../middleware/validators');

router.post('/run', validateStrategy, (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, error: 'Erro ao executar backtest' });
  }
});
